...
const envVarsInputs = document.getElementById('env-vars-inputs');
const deploymentProgress = document.getElementById('deployment-progress');
const statusBadge = document.getElementById('status-badge');
const deploymentLogs = document.getElementById('deployment-logs');
...
function renderDeploymentPlans() {
    ...
    <div class="plan-card">
      ...
      ${plan.notes.length > 0 ? `<div style="margin-top: 10px;"><strong>Notes:</strong><ul style="margin-left: 20px;">${plan.notes.map((note) => `<li>${note}</li>`).join('')}</ul></div>` : ''}
...
packageSelect.addEventListener('change', () => {
    ...
    vendorSelect.innerHTML = '<option value="">-- Select a vendor --</option>';
    plan.deploymentOptions.forEach((option) => {
...
function getVendorRequiredEnvVars(vendor) {
    const vendorEnvVars = {
